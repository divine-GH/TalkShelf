...
- **检索页/记录页用户消息重复渲染**（P1）：提交后「手动渲染 + 轮询按 id 重放」双路径导致
  刚落库的 user 消息画两次（检索会话 #16 实测 DOM 6 节点 vs DB 5 行）；改为用户消息统一
  由轮询按消息 id 渲染，仅保留「思考中…」即时反馈。
- **抓取来源 URL 携带分享追踪参数**（P2）：`b23.tv` 短链重定向后的长链携带
  `buvid`/`mid`/`share_*`/`spmid` 等设备指纹与用户标识参数，随 `Fetched` 头落库并展示为
  「来源 ↗」链接；新增 `fetch.strip_tracking_url` 在 `fetched_message` 源头清洗（业界通用
  追踪参数黑名单 + B 站/小黑盒等站点参数白名单），下游落库/展示/LLM 上下文一次全干净。
- **检索材料框近重复**（P3）：同一轮自动注入 + LLM `note_search` 重查多次的检索结果此前
  各渲染一个折叠条（标题雷同、编号错位）；现合并为一个「检索材料」折叠条（每块带 query
  标签，标题汇总「共 N 条来源 · M 次检索」），prompt 引导 LLM 重查换关键词。

### Notes

- 以上三项均来自「检索 — TalkShelf.html」页面快照核查（会话 #16），验收通过后发版；
  其余待办（P4~P7：材料片段质量、前端状态清理、计数口径）记录在《已知问题.md》#5~#8。
