...
- **Web 检索页**（对话式，§36）：向量 + FTS + RRF 融合 + 材料层兜底召回 → LLM 对话式作答带引用（点击跳转原笔记）；多轮追问，LLM 带 `note_search` 工具可主动补检/重检，意图词命中时联网搜索补充
- **检索会话**：历史会话可回看/继续/删除（替代旧的「检索记录」单条历史；`SEARCH_HISTORY_LIMIT` 已移除，新增 `SEARCH_TOOL_MAX_ROUNDS` 控制工具循环上限）
...
- 143 个 pytest 用例（LLM/embedding 全 mock），真实 DeepSeek + Ollama 端到端冒烟通过
