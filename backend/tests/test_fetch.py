...
def test_extract_falls_back_to_metadata_when_no_body():
    ...
    md, title = fetch._extract(html, "https://b23.tv/abc")
...
def test_fetched_message_format():
    ...
    result = fetch.FetchResult(url="https://a.b/c", status=200, title="t",
                               markdown="x" * (fetch.config.FETCH_TEXT_LIMIT + 100), truncated=True)
