...
def test_schema_creates_all_tables(conn):
    ...
    assert {"notes", "conversations", "messages", "tags", "entities", "embeddings",
            "note_materials", "notes_fts", "materials_fts"} <= tables
...
def test_foreign_keys_cascade(conn):
    ...
    assert conn.execute("SELECT COUNT(*) FROM conversations WHERE note_id = ?", (nid,)).fetchone()[0] == 0
...
def test_trigram_fts_sync_and_search(conn):
    ...
    rows = conn.execute("SELECT rowid FROM notes_fts WHERE notes_fts MATCH ?", ('"上传大文件"',)).fetchall()
    ...
    db.fts_sync(conn, nid)
    conn.commit()
    rows = conn.execute("SELECT rowid FROM notes_fts WHERE notes_fts MATCH ?", ('"部署配置"',)).fetchall()
    assert [r["rowid"] for r in rows] == [nid]

    # 2 字词 trigram 不命中（设计文档 §4 关键点 3：靠 LIKE 兜底，检索层测试覆盖）
    rows = conn.execute("SELECT rowid FROM notes_fts WHERE notes_fts MATCH ?", ('"上传"',)).fetchall()
    assert rows == []
...
def test_materials_fts(conn):
    ...
    conn.commit()
    rows = conn.execute("SELECT rowid FROM materials_fts WHERE materials_fts MATCH ?", ('"网页正文"',)).fetchall()
