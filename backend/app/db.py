...
-- 检索会话（§36 对话式检索）：复用 conversations/messages 表，status='search' 区分；
-- 注意：检索会话可删除（DELETE 端点），历史会话仅作回看，不自动裁剪（与记录对话草稿一致）。
...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    # §36：检索页从「单轮 + 检索记录」升级为「对话式检索 + 检索会话历史」——
    # 旧 search_history 表（问题+答案）按决策删除（功能被检索会话取代，数据无迁移价值）
